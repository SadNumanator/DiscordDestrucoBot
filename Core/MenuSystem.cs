...
namespace DiscordDestrucoBot
{
    public class MenuSystem
    {
        ...
        public void AddMenu(string menutitle, string previousMenu = null, System.Drawing.Color? color = null)
        {//Adds the menu title to the Dictionary of menus as the value and title.
            if (color == null)
                color = System.Drawing.Color.FromArgb(49,79,79);
            Menus.Add(menutitle, new string[] { previousMenu, color.ToString()});
        ...
        public void AddFieldInMenu(string parent, string menuToOpen, string title, string description)
        {
            ...
            if (MenuFields.ContainsKey(parent + menufieldcount))
                new Exception("Somehow the menu already contained a key of " + parent + menufieldcount);
        ...
        public void ReturnCurrentMenu(out string menutitle, out string[][] menufields, out int menufieldcount)
        {
            if (!Menus.ContainsKey(CurrentMenu))
                new Exception("Tried to get menu key which did not exist");
        ...
        public void SetCurrentMenu(string menutitle)
        {
            if (Menus.ContainsKey(menutitle))
                ...
                new Exception("In SetCurrentMenu() the menu provided does not exist");
        }
        ...
        public int[] GetCurrentMenuColor()
        {
            ...
            var p = menuData[1].Split(new char[] { ',', ']' });
        ...
        {   //Initialize the EmbedBuilder
            ...
            while (true)
            {
                int delaytime = 500;//The update time. The bigger it is the slower this updates
                ...
                */
                ...
                if (oldCurrentMenu != CurrentMenu)
                {
                    ...
                    ReturnCurrentMenu(out menutitle, out menufields, out menufieldcount);
                    //Create the embedbuilder
                    builder = new EmbedBuilder();
                    CurrentMenuEmbedBuilder(builder, out menufieldcount);
